...
function completeChild(attestationDir: string): { runId: string; childIndex: number; attestationPath: string } {
  const runId = "e2e-engineering-manager";
  const childIndex = 3;
  const child = new PersonaChildRuntime({
    identity: { runtimeName: "persona-team.engineering-manager", runId, childIndex },
    personaPath,
    workspace: root,
    attestationDir,
  });
  ...
  if (!completed.ok || !completed.attestationPath) throw new Error(completed.message);
  return { runId, childIndex, attestationPath: completed.attestationPath };
}
...
test("parent accepts a persisted host attestation and rejects reused run identity", async () => {
  ...
  const persisted = JSON.parse(readFileSync(childRun.attestationPath, "utf8")) as { schema: string; status: string; runId: string; childIndex: number };
  ...
  const accepted = await runPersona({
    packageRoot: root,
    workspace: root,
    delegate: async () => ({ ...childRun, output: "Plan artifact produced.", ordinaryAccepted: true }),
  ...
  const reused = await runPersona({
    ...
    delegate: async () => ({ ...childRun, ordinaryAccepted: true }),
...
test("failure attestation persists across lifecycle shutdown and is rejected by the parent", async () => {
  ...
  const child = new PersonaChildRuntime({
    identity: { runtimeName: "persona-team.engineering-manager", runId: "failed-lifecycle", childIndex: 4 },
    personaPath,
    workspace: root,
    attestationDir,
  });
  ...
  const accepted = await runPersona({
    packageRoot: root,
    workspace: root,
    delegate: async () => ({ runId: "failed-lifecycle", childIndex: 4, attestationPath: persisted.path, ordinaryAccepted: true }),
